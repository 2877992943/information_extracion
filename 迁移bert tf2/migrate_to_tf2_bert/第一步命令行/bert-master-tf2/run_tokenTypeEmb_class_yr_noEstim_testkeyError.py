...
def convert_examples_to_features1(examples,
  ...
  for (example_index, example) in enumerate(examples):
    ...
    tf.compat.v1.logging.info("char: %s" % (' '.join([str(w) for w in input_ids])))
    tf.compat.v1.logging.info("char mask: %s" % (' '.join([str(w) for w in input_mask])))
    tf.compat.v1.logging.info("pos: %s" % (' '.join([str(w) for w in segment_ids])))
    tf.compat.v1.logging.info("predicate: %s" % (' '.join([str(w) for w in predicate_ids])))
    tf.compat.v1.logging.info("predicate number: %d" % (num_y))
...
class FeatureWriter(object):
  ...
  def process_feature(self, feature):
    ...
    #     "input_ids": tf.FixedLenFeature([301], tf.int64),
    #     "input_mask": tf.FixedLenFeature([301], tf.int64),
    #     "segment_ids": tf.FixedLenFeature([301], tf.int64),
    # }
    # name_to_features['target_ids'] = tf.FixedLenFeature([52], tf.int64)
    # name_to_features["num_of_target"] = tf.FixedLenFeature([], tf.int64)
    # example = tf.parse_single_example(ss, name_to_features)
    # sess=tf.Session()
    # sess.run(example)`
    print ('')
...
def main(_):
  ...
  if FLAGS.do_train:
    ...
    n=next_element['input_mask']
    n=next_element['input_ids']
    #n=tf.constant([1,2,4])
    tensor = sess.run(fetches=n)
...
if __name__ == "__main__":
  ...
  if local_flag == True:
      ...
      OLD_BERT_MODEL_DIR = '../../../download_model_cn/chinese_L-12_H-768_A-12/'
      XIAOBAI_DATA_DIR = '../../bert_demo_data/knowlege'
      FLAGS.data_dir = '../../tmp/'
      FLAGS.output_dir = '../../model_specifiedTask/'
  ...
  ####
  FLAGS.train_file=os.path.join(XIAOBAI_DATA_DIR,'tmp','dev1.json') # train1.json
