...
def body_fn(enc,cls_num,cls_multi,next_ele,is_training):
    # input -> encode -> seq logits
    # logits -> cls1 cls2
    input_ids=next_ele['input_ids']
    ...
    if is_training==True:
        ...
        ###
        logits_n=cls_num(tf.reduce_sum(seq_out,axis=1))
        logits_multi=cls_multi(tf.reduce_sum(seq_out,axis=1))
        return logits_n,logits_multi



def calc_loss(target_ids,num_target,logits_multi,logits_num):# [8 25]][8,][8 52category][8,25]
    ##### loss 1 number of target
    y = tf.one_hot(num_target, depth=FLAGS.max_seq_length_y)  # [batch 25] 25=max num of y
    prob_num_target = tf.nn.softmax(logits_num, axis=-1) #[8 25]
    loss1 = -tf.reduce_mean(input_tensor=tf.reduce_sum(input_tensor=y * tf.math.log(prob_num_target + eps), axis=-1))
    ##### loss2   multi label
    y = tf.reduce_sum(input_tensor=tf.one_hot(target_ids, depth=FLAGS.target_vocab_size), axis=1)[:, 1:]
    # [batch ,numClass25, vocabsz50]->[batch vocabsz] -> remove padding
    prob_y = tf.nn.sigmoid(logits_multi)[:, 1:]  # [batch vocabsz]
    loss2 = y * tf.math.log(eps + prob_y) + (1 - y) * tf.math.log(1. - prob_y + eps)
    loss2 = tf.reduce_mean(input_tensor=-loss2)
    return loss1+loss2
...
##########  input data
  ...
  #### multi class layer
  cls_num=Dense(name='num_cls',units=FLAGS.max_seq_length_y)
  cls_multi=Dense(name='multi_cls',units=FLAGS.target_vocab_size)
  ...
  for step, next_ele in enumerate(inp_dataset):
      with tf.GradientTape() as tape:
          ...
          logitsn,logits_multi = body_fn(bertEncode, cls_num,cls_multi, next_ele,is_training)
          loss=calc_loss(next_ele['target_ids'],next_ele['num_of_target'],logits_multi,logitsn)

          #####
          if step%10==0:
              print('step %d loss %f'%(step,loss.numpy()))
      #####
      weights_ = tuple(bertEncode.trainable_variables) + \
                 tuple(cls_multi.trainable_weights)+\
                 tuple(cls_num.trainable_weights)
...
if __name__ == "__main__":
  ...
  FLAGS.train_file=os.path.join(XIAOBAI_DATA_DIR,'tmp',f1[0])
