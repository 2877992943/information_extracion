...
def read_knowledge_example(inputfile,is_training):
    ...
    for line in reader.readlines():
        ...
        examples.append({'text':text,'tag':postag,
                         'yseq':d['yseq'],'predicate':d['predicate']})
    return examples
...
def convert_examples_to_features1(examples,
  ...
  for (example_index, example) in enumerate(examples):
    ...
    #query_tokens = tokenizer.tokenize(example.question_text)
    charll,posll,yseqll=example['text'],example['tag'],example['yseq']
    ...
    # zero pad y seq
    while len(yseq_ids) < max_seq_length_y:
      yseq_ids.append(0)

    assert len(yseq_ids) == max_seq_length_y



    ### num of target
    #num_y=len([yi for yi in yseq_ids if yi!=0])
    ...
    #         "input_mask":input_mask,
    #         "segment_ids":segment_ids,
    #         'input_str':''.join(charll),
    #         'token_type_str':' '.join(posll),
    #         'predicate':' '.join(yseqll)}
    #     yield d
...
if __name__ == "__main__":
  ...
  ####
  f1=['dev11.json','train1.json']
  FLAGS.train_file=os.path.join(XIAOBAI_DATA_DIR,'tmp',f1[0])
  FLAGS.vocab_file=os.path.join(XIAOBAI_DATA_DIR,'dict_v1231','vocab_bertEncode_3w.txt')

  ### target seq


  FLAGS.target_vocab=os.path.join(XIAOBAI_DATA_DIR,'dict_v1231','rela_label_y.dict')

  FLAGS.target_vocab_size = 4551
  ### pos vocab
  ...
  FLAGS.max_seq_length_y=130 # 比最长128多
  FLAGS.max_seq_length=130
