...
# bert_m_inp=[Input(shape=(None,None), name='token'),
#             Input(shape=(None,None), name='segm')]
...
#subject_model = Model(bert_m_inp, subject_preds) #
...
#                               )
#     ##
#     train_model.save_weights('./model_step1/best_model.weights')

#
# else:

    train_model.load_weights('./best_model1.weights')
    #datall=load_data('../spo_data/test50.json')
    datall=load_data('../spo_data/dev1.json')
    #rst=evaluate(datall)
    #print (rst)

    writer=open('./step1_rst.json','w')
    cnt=0
    for d in datall:
        s_p_ll=extract_spoes(d['text'])
        d['s_p']=s_p_ll
