...
output = LayerNormalization(conditional=True,name='specialNorm')([output, subject_predicate]) #[? ? 768] mean std 依赖sub-hid
output = Dense(units=2,
...
object_model = Model(bert.model.inputs + [subject_ids, predicate_id], object_preds)
...
def evaluate(data):
    ...
    X, Y, Z = 1e-10, 1e-10, 1e-10
    f = codecs.open('dev_pred2.json', 'w', encoding='utf-8')
    pbar = tqdm()
    for d in data:
        # R = set([SPO(spo) for spo in extract_spoes(d['text'])])
        # T = set([SPO(spo) for spo in d['spo_list']])
...
if __name__ == '__main__':
    ...
    train_model.save_weights('/models/best_model2.weights') #
